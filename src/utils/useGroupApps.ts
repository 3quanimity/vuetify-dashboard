...
export default function groupByAppAndCountry(data: any) {
    ...
    for(const entry of data) {
        ...
        const revenue = parseInt(entry.revenue); // rounding for simplification
        const format = entry.format;

        // if key dosnt exist in map, create it
        if (!map.has(`${app} ${os}`)) {
            map.set(`${app} ${os}`, {
                data: entry,
        ...
        // else agglomerate new entry data with existing data (exisitng map key)
        } else {
